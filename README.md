...
function removerColab(id){ if(confirm("Excluir colaborador?")){ colaboradores=colaboradores.filter(x=>x.id!==id); pontos=pontos.filter(p=>p.idColab!==id); renderAll(); salvarFirebase(); }}
...
/* LIMPAR */
limparTodosBtn.onclick=()=>{ if(confirm("Limpar todos os pontos?")){ pontos=[]; renderEntradasSaidas(); salvarFirebase(); } };
</script>
